...
let rec eval_expression expr =
  ...
  | Var v -> (
    ...
    | Some x -> x
    | None -> failwith "Variable non initialisee"
  )
  | Puissance (e1, e2) -> (eval_expression e1) ** (eval_expression e2)
  | Fact e -> (
    let rec fact n =
      if n = 0 then 1
      else n * fact (n - 1)
    in
    float_of_int (fact (int_of_float (eval_expression e)))
  )
...
let rec eval_instruction instr =
  ...
  | Imprime l -> (
      ...
      | h::t -> 
          ...
          | _ -> Printf.printf "%f" (eval_expression h) ; eval_instruction (Imprime t)
  )
  | SiAlors(e1, r, e2, i) -> 
    if eval_relop e1 r e2 then eval_instruction i
  | Assigne (v, e) -> 
      let res = eval_expression e in
      Hashtbl.replace vars v res
  | AssigneList (v, e) -> 
    let tmp = Hashtbl.copy vars in
    let pairs = List.combine v e in
    List.iter (fun (v, e) ->
      let res = eval_expression e in
      Hashtbl.replace vars v res
    ) pairs;
    Hashtbl.iter (fun k v -> Hashtbl.replace vars k v) tmp
  ...
  | Entree l -> (
      ...
      | h::t -> 
        match h with
        | Var v -> Hashtbl.replace vars v (entree v) ; eval_instruction (Entree t)
