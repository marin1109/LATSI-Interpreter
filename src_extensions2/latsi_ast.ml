...
type instruction =
  ...
  | AssigneList of (char list) * (expression list)
...
let rec print_instruction = function
  ...
  | AssigneList (v, e) -> List.iter (fun var -> Printf.printf "%c" var; Printf.printf " , ") v; Printf.printf " = "; List.iter (fun expr -> print_expression expr; Printf.printf " , ") e
